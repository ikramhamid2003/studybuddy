...
| `POST` | `/api/generate/` | JWT | **Single unified generation endpoint** — runs any tool and persists the result. Body: `topic`, `type` (`explain` \| `summarize` \| `quiz` \| `flashcards` \| `chat`) plus optional per-tool params: `level` (explain), `format` (summarize), `num_questions` + `difficulty` (quiz), `num_cards` (flashcards), `history` (chat) |
| `GET` | `/api/generations/` | JWT | List persisted generations, newest first (`?type=` filter) |
