...
def test_explain_view(mock_query, auth_client):
    ...
    resp = auth_client.post(
        "/api/explain/", {"topic": "react", "level": "beginner"}, format="json"
    )
...
def test_summarize_view(mock_query, auth_client):
    ...
    resp = auth_client.post(
        "/api/summarize/",
        {
            "notes": "This is a long piece of study notes that is at least twenty characters long.",
            "format": "bullets",
        },
        format="json",
    )
...
def test_quiz_view(mock_query, auth_client):
    from study_api.schemas import QuizResponse
    ...
    resp = auth_client.post(
        "/api/quiz/",
        {"topic": "react", "num_questions": 1, "difficulty": "easy"},
        format="json",
    )
    assert resp.status_code == 200
...
def test_flashcards_view(mock_query, auth_client):
    from study_api.schemas import FlashcardsResponse
    ...
    resp = auth_client.post(
        "/api/flashcards/", {"topic": "react", "num_cards": 5}, format="json"
    )
    assert resp.status_code == 200
...
def test_chat_view(mock_chat, auth_client):
    mock_chat.return_value = "hello there student"
